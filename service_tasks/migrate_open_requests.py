...
from helpers.circulation_helper import CirculationHelper, LegacyLoan, TransactionResult, LegacyRequest
from helpers.custom_dict import InsensitiveDictReader
from service_tasks.service_task_base import ServiceTaskBase


class MigrateOpenRequests(ServiceTaskBase):
    ...
    def __init__(self, folio_client, args):
        ...
        super().__init__(folio_client, "MigrateOpenRequests", self.reports_folder)
        ...
        with open(args.open_requests_file, 'r', encoding="utf-8") as requests_file:
            self.valid_legacy_requests = list(
                self.load_and_validate_legacy_requests(InsensitiveDictReader(requests_file, dialect="tsv")))
            logging.info(f"Loaded and validated {len(self.valid_legacy_requests)} requests in file")
    ...
    def do_work(self):
        ...
        for num_requests, legacy_request in enumerate(self.valid_legacy_requests[self.starting_point:]):
            ...
            try:
                ...
                if patron and item:
                    ...
                    res_request = self.circulation_helper.create_request(self.folio_client, legacy_request)
                    if res_request:
                        ...
                        logging.error(f'Unsuccessfully processed requests.ItemBarcode {legacy_request.item_barcode} Status: {item["status"]["name"]}')
                        self.add_stats(f'Unsuccessful statuses: {item["status"]["name"]}')
                        self.add_stats("Unsuccessfully processed requests")
                        self.failed_requests.add(legacy_request)

            except Exception as ee:  # Catch other exceptions than HTTP errors
                logging.info(f"Error in row {num_requests}  Item barcode: {legacy_request.item_barcode} "
    ...
    def add_arguments(parser):
        ...
        ServiceTaskBase.add_argument(parser,
                                     "open_requests_file", help="File to TSV file containing Open Loans",
                                     widget="FileChooser"
                                     )
    ...
    def load_and_validate_legacy_requests(self, loans_reader):
        ...
        for legacy_request_count, legacy_request_dict in enumerate(loans_reader):
            try:
                legacy_request = LegacyRequest(legacy_request_dict, legacy_request_count)
        ...
        logging.info(f"Done validating {legacy_request_count} legacy requests with {num_bad} rotten apples")
    ...
    def wrap_up(self):
        ...
        file_path = os.path.join(self.results_folder, f'failed_requests_{time.strftime("%Y%m%d-%H%M%S")}.tsv')
