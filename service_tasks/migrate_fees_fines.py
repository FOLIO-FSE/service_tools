...
class MigrateFeesAndFines(ServiceTaskBase):
        ...
        for num_fees, legacy_fee in enumerate(self.valid_legacy_fees[self.starting_point:]):
            ...
            try:
                ...
                account_payload = {
                    ...
                    "materialType": patron_item.get("item", {}).get("materialType", {}).get("name", ""),
                    "location": patron_item.get("item", {}).get("effectiveLocation", {}).get("name", ""),
                    "metadata": self.folio_client.get_metadata_construct(),
                    "userId": patron_item.get("user", {}).get("id", ""),
                    "itemId": patron_item.get("item", {}).get("id", ""),
                    "materialTypeId": patron_item.get("item", {}).get("materialType", {}).get("id", ""),
    ...
    def load_and_validate_legacy_fees(self, fees_reader):
        ...
        logging.info(f"Done validating {legacy_fees_count} legacy FeeFines with {num_bad} rotten apples")
